...
### CDIP - Costal Data Information Program

Data courtesy of {% raw %}<a href="https://cdip.ucsd.edu/">CDIP</a>.{% endraw %}

[CDIP](https://cdip.ucsd.edu/m/deployment/station_view/) offers data from a pubic [THREDDS](http://thredds.cdip.ucsd.edu/thredds/catalog/cdip/archive/catalog.html) for their historic buoy data sets. 

The DLC Generator tool automates the process of:
1. Interacting with the THREDDS server and gathering all years of available data.
2. Takes the wave spectral density data ({% raw %}$$m^2/H_z$${% endraw %} indexed by {% raw %}$$H_z$${% endraw %}) from the buoy and uses [MHKiT](https://mhkit-software.github.io/MHKiT/index.html) to calculate spectral parameters {% raw %}$$H_{mo}$${% endraw %}, {% raw %}$$T_p$${% endraw %}, and {% raw %}$$T_e$${% endraw %}.
3. Removing duplicate time indexes, null values, and known corrupt values from the final time series data ([details](/theory/qualityControl)).

### NDBC - NOAA National Data Buoy Center

[NDBC](https://www.ndbc.noaa.gov/) provides many different historical data sets. See their [index](https://www.ndbc.noaa.gov/data/historical/) for more information.
...
Virtual buoys are only available in the regions around the West and East Coasts of United States.
